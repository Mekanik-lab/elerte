...
function handlePostActions($database, $sessionUserId)
{
    ...
    if (isset($_POST["changeRfid"])) {
        handleChangeRfid($database, $sessionUserId);
        return;
    }
    ...
    if (isset($_POST["adminChangeUserRfid"])) {
        handleAdminChangeRfid($database, $sessionUserId);
        return;
    }
    ...
    if (isset($_POST["addCategoryDictionary"])) {
        handleAddCategoryDictionary($database);
        return;
    }

    if (isset($_POST["addLocationDictionary"])) {
...
{
    $sql = "SELECT id, login, imie, nazwisko, status_konta, rola, rfid FROM uzytkownicy WHERE id = ? LIMIT 1";
    $stmt = mysqli_prepare($database, $sql);
    mysqli_stmt_bind_param($stmt, "i", $userId);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
function rfidExistsForAnotherUser($database, $rfid, $excludedUserId = 0)
{
    ...
    $sql = "SELECT id FROM uzytkownicy WHERE rfid = ? AND id <> ? LIMIT 1";
    $stmt = mysqli_prepare($database, $sql);
    mysqli_stmt_bind_param($stmt, "si", $rfid, $excludedUserId);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
function handleAddProduct($database, $sessionUserId)
{
    ...
    saveDictionaryValue($database, "slownik_kategorie", $category);
    saveDictionaryValue($database, "slownik_lokalizacje", $adress);
...
function handleEditProduct($database, $sessionUserId)
{
    ...
    if ($id <= 0 || $name === "" || $quantity < 0 || $unit === "") {
        setFlashMessage("Nieprawidłowe dane produktu.", "danger");
        return;
    }
...
function handleChangeRfid($database, $sessionUserId)
{
    if (($_SESSION["login"] ?? "") !== "AdminSystemu") {
        setFlashMessage("Nie masz uprawnień do zmiany własnego RFID.", "danger");
        redirect("index.php");
    }
    ...
    if (!$user || $user["status_konta"] !== "aktywne") {
        setFlashMessage("Nie można zmienić RFID dla nieaktywnego konta.", "danger");
        redirect("index.php");
    }

    $newRfid = trim((string) getPostText("newRfid"));

    if ($newRfid === "") {
        setFlashMessage("Kod RFID nie może być pusty.", "danger");
        redirect("index.php");
    }

    if (rfidExistsForAnotherUser($database, $newRfid, $sessionUserId)) {
        setFlashMessage("Taki kod RFID jest już przypisany do innego użytkownika.", "danger");
        redirect("index.php");
    }

    $sql = "UPDATE uzytkownicy SET rfid = ? WHERE id = ?";
    $stmt = mysqli_prepare($database, $sql);
    mysqli_stmt_bind_param($stmt, "si", $newRfid, $sessionUserId);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);

    setFlashMessage("RFID został zmieniony.", "success");
    redirect("index.php");
}
...
function handleAdminChangeRfid($database, $sessionUserId)
{
    if (!canManageUsers()) {
        setFlashMessage("Nie masz uprawnień do tej operacji.", "danger");
        redirect("index.php");
    ...
    if (!canChangeTargetRfid($targetUser)) {
        setFlashMessage("Nie można zmienić RFID temu użytkownikowi.", "danger");
        redirect("index.php");
    }
    ...
    if (rfidExistsForAnotherUser($database, $newRfid, $targetUserId)) {
        setFlashMessage("Taki kod RFID jest już przypisany do innego użytkownika.", "danger");
        redirect("index.php");
    }
    ...
    $sql = "UPDATE uzytkownicy SET rfid = ? WHERE id = ?";
    $stmt = mysqli_prepare($database, $sql);
    mysqli_stmt_bind_param($stmt, "si", $newRfid, $targetUserId);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);

    setFlashMessage("RFID użytkownika został zmieniony.", "success");
    redirect("index.php");
}
...
function handleAddCategoryDictionary($database)
{
    requireAdmin();

    $value = getPostText("dictionaryValue");

    if ($value === "") {
        setFlashMessage("Nazwa kategorii nie może być pusta.", "danger");
        redirect("index.php");
    }
...
function handleAddLocationDictionary($database)
{
    requireAdmin();
    ...
    if ($value === "") {
        setFlashMessage("Nazwa lokalizacji nie może być pusta.", "danger");
        redirect("index.php");
    }
...
<body 
data-user-fullname="<?= escapeHtml($currentUserFullName) ?>"
data-user-role="<?= escapeHtml($_SESSION['rola'] ?? '') ?>"
data-user-login="<?= escapeHtml($_SESSION['login'] ?? '') ?>">
