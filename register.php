...
if (isPostRequest()) {
    ...
    if ($error === "" && rfidExists($database, $formData["rfid"])) {
        $error = "Taki kod RFID jest już przypisany do innego użytkownika.";
    }
...
{
    $requiredFields = ["login", "password", "repeatPassword", "imie", "nazwisko", "rola"];
...
function rfidExists($database, $rfid)
{
    if ($rfid === "") {
        return false;
    }

    $sql = "SELECT id FROM uzytkownicy WHERE rfid = ? LIMIT 1";
    $stmt = mysqli_prepare($database, $sql);
...
{
    $sql = "INSERT INTO uzytkownicy (login, rfid, haslo, imie, nazwisko, status_konta, rola)
            VALUES (?, ?, ?, ?, ?, 'aktywne', ?)";

    $stmt = mysqli_prepare($database, $sql);
    $hashedPassword = password_hash($data["password"], PASSWORD_DEFAULT);

    $rfid = $data["rfid"] !== "" ? $data["rfid"] : null;
...
<header class="bg-warning border-bottom shadow-sm">
    <div class="container-fluid py-3">
        <div class="d-flex align-items-center justify-content-between">
            ...
            <a id="backToUsers" href="index.php" class="btn btn-light">Powrót</a>
        </div>
    </div>
</header>
    ...
    <div class="card shadow border-0" style="max-width: 420px; width: 100%;">
        ...
        <div class="card-body p-4">
            <form method="POST" action="register.php" class="d-grid gap-3">
                ...
                <div>
                    <label for="selectRole" class="form-label fw-semibold">Wybierz rolę:</label>
                ...
                <div>
                    <label for="rfid" class="form-label fw-semibold">Karta RFID (opcjonalnie)</label>
                    <input type="text" id="rfid" name="rfid" class="form-control">
                    <div class="form-text">Przyłóż kartę do czytnika albo wpisz kod ręcznie.</div>
                </div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (backBtn) {
        backBtn.addEventListener("click", () => {
            localStorage.setItem("lastSection_global", "uzytkownicy");
        });
    }
});
</script>
