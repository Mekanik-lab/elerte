...
function enforceSessionTimeoutForAjax()
{
    $timeoutSeconds = 300;
...
function renderUsersActions($row)
{
    ...
    if (canChangeTargetRfid($targetUser)) {
        echo "<button class='changeUserRfidBtn btn btn-info btn-sm me-1'>Zmień RFID</button>";
        $hasActions = true;
    }
...
function getFilterSessionKey($table)
{
    return "table_filters_" . $table;
}

function savePostedFiltersToSession($table)
{
    if (!isset($_SESSION["tableFilters"])) {
        $_SESSION["tableFilters"] = [];
    }

    $allowedFieldsByTable = [
        "magazyn" => [
            "table", "page", "isFilterRequest",
            "searchProduct", "productName", "addedBy", "productCategory",
            "productAdress", "productComments", "quantityFrom", "quantityTo",
        ...
        "wydania" => [
            "table", "page", "isFilterRequest",
            "issuedBy", "productName", "reason", "issueDate",
            "quantityFrom", "quantityTo", "sortColumn", "sortOrder"
        ],
        "uzytkownicy" => [
            "table", "page", "isFilterRequest",
            "login", "fullName", "status", "role", "sortColumn", "sortOrder"
        ],
        "inwentaryzacje" => [
            "table", "page", "isFilterRequest",
            "inventoriedBy", "inventoryNumber", "createdDate",
            "approvedDate", "approved", "sortColumn", "sortOrder"
        ],
        "inwentaryzacja_pozycja" => [
            "table", "page", "isFilterRequest",
    ...
    $allowedFields = $allowedFieldsByTable[$table] ?? [];
    $cleanPost = [];

    foreach ($allowedFields as $field) {
        if (isset($_POST[$field])) {
            $cleanPost[$field] = $_POST[$field];
        }
    }

    $_SESSION["tableFilters"][getFilterSessionKey($table)] = $cleanPost;
}

function getSavedFiltersFromSession($table)
{
    return $_SESSION["tableFilters"][getFilterSessionKey($table)] ?? [];
}

function clearSavedFiltersFromSession($table)
{
    if (isset($_SESSION["tableFilters"][getFilterSessionKey($table)])) {
        unset($_SESSION["tableFilters"][getFilterSessionKey($table)]);
    }
}

function isFilterRequest()
{
    return isset($_POST["isFilterRequest"]) && $_POST["isFilterRequest"] === "1";
}

function shouldClearFiltersRequest()
{
    return isset($_POST["clearFilters"]) && $_POST["clearFilters"] === "1";
}

function applySavedFiltersToPost($table)
{
    $savedFilters = getSavedFiltersFromSession($table);

    if (!empty($savedFilters)) {
        $_POST = array_merge($savedFilters, $_POST);
    }
}
...
function handleTableRequest($database, $table, $page, $perPage, $offset)
{
    ...
    if (isPostRequest() && isFilterRequest()) {
        savePostedFiltersToSession($table);
    }
    ...
    if ($table === "historia_operacji" && hasHistoryFilters()) {
        handleHistoryFilter($database, $page, $perPage, $offset);
    }

    if ($table === "wydania" && hasIssuesFilters()) {
        handleIssuesFilter($database, $page, $perPage, $offset);
    }

    if ($table === "uzytkownicy" && hasUsersFilters()) {
        handleUsersFilter($database, $page, $perPage, $offset);
    }

    if ($table === "inwentaryzacje" && hasInventoriesFilters()) {
...
function buildHistoryFilterData()
{
    ...
    if (getPostText("fullName") !== "") {
        $where[] = "CONCAT(uzytkownicy.imie, ' ', uzytkownicy.nazwisko) LIKE ?";
        $params[] = "%" . getPostText("fullName") . "%";
        $types .= "s";
    }
...
function buildIssuesFilterData()
{
    ...
    if (getPostText("issuedBy") !== "") {
        $where[] = "CONCAT(uzytkownicy.imie, ' ', uzytkownicy.nazwisko) LIKE ?";
        $params[] = "%" . getPostText("issuedBy") . "%";
        $types .= "s";
    }
...
function buildInventoriesFilterData()
{
    ...
    if (getPostText("inventoriedBy") !== "") {
        $where[] = "CONCAT(uzytkownicy.imie, ' ', uzytkownicy.nazwisko) LIKE ?";
        $params[] = "%" . getPostText("inventoriedBy") . "%";
        $types .= "s";
    }
